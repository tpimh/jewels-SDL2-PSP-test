...
int main(int argc, char *argv[]) {
    ...
    if (FT_Init_FreeType(&ft_library) != 0) return 1;

    /* Load our fonts */
    FT_Face ft_face[NUM_EXAMPLES];
    if (FT_New_Face(ft_library, "fonts/DejaVuSerif.ttf", 0, &ft_face[ENGLISH]) != 0) return 1;
    if (FT_Set_Char_Size(ft_face[ENGLISH], 0, ptSize, device_hdpi, device_vdpi ) != 0) return 1;
    ftfdump(ft_face[ENGLISH]); // wonderful world of encodings ...
    force_ucs2_charmap(ft_face[ENGLISH]); // which we ignore.

    if (FT_New_Face(ft_library, "fonts/amiri-0.104/amiri-regular.ttf", 0, &ft_face[ARABIC]) != 0) return 1;
    if (FT_Set_Char_Size(ft_face[ARABIC], 0, ptSize, device_hdpi, device_vdpi ) != 0) return 1;
    ftfdump(ft_face[ARABIC]);
    force_ucs2_charmap(ft_face[ARABIC]);

    if (FT_New_Face(ft_library, "fonts/fireflysung-1.3.0/fireflysung.ttf", 0, &ft_face[CHINESE]) != 0) return 1;
    if (FT_Set_Char_Size(ft_face[CHINESE], 0, ptSize, device_hdpi, device_vdpi ) != 0) return 1;
        ...
        for (int i=0; i < NUM_EXAMPLES; ++i) {
            ...
            */
            ...
            if (HB_DIRECTION_IS_HORIZONTAL(hb_buffer_get_direction(buf))) {
                ...
                /* draw the baseline */
                hline(surface, x, x + bbox_w, y, 0x0000ff00);
            }
            ...
            if (HB_DIRECTION_IS_VERTICAL(hb_buffer_get_direction(buf))) {
                ...
                vline(surface, y, y + bbox_h, x, 0x0000ff00);
            }

            /* +1/-1 are for the bbox borders be the next pixel outside the bbox itself */
            hline(surface, left - 1, right + 1, top - 1, 0x00ff0000);
            hline(surface, left - 1, right + 1, bottom + 1, 0x00ff0000);
            vline(surface, top - 1, bottom + 1, left - 1, 0x00ff0000);
            vline(surface, top - 1, bottom + 1, right + 1, 0x00ff0000);
            ...
            stuffbaton.first_pixel = surface->pixels;
            stuffbaton.last_pixel = (uint32_t *) (((uint8_t *) surface->pixels) + surface->pitch*surface->h);
            stuffbaton.pitch = surface->pitch;
            stuffbaton.rshift = surface->format->Rshift;
            stuffbaton.gshift = surface->format->Gshift;
            stuffbaton.bshift = surface->format->Bshift;
            ...
            for (unsigned j=0; j < glyph_count; ++j) {
                if ((fterr = FT_Load_Glyph(ft_face[i], glyph_info[j].codepoint, 0))) {
                    ...
                    if (ft_face[i]->glyph->format != FT_GLYPH_FORMAT_OUTLINE) {
                        ...
                        stuffbaton.pixels = (uint32_t *)(((uint8_t *) surface->pixels) + gy * surface->pitch) + gx;
